...
parser.add_argument('--datapath', type=str, default='assets/data', help="input data path")
parser.add_argument('--seed', type=int, default=1234, help="random seed")
parser.add_argument('--batch_size', type=int, default=64, help="batch size")
parser.add_argument('--mode', type=str, default='eval', help='eval | targeted_attack | untargeted_attack')
...
def evaluate(hps, data_X, data_y, eval_once=True):
    """Eval loop."""
    images = tf.compat.v1.placeholder(tf.float32, shape=(None, args.image_size, args.image_size, args.channels))
    ...
    if args.classifier == "madry":
        ...
        net = tf.compat.v1.make_template('net', resnet_template, hps=hps) if args.classifier == 'resnet' else \
    ...
    saver = tf.compat.v1.train.Saver(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='net'))
    ...
    while True:
        ...
        if not (ckpt_state and ckpt_state.model_checkpoint_path):
            print(f'[!] No model to eval yet at {save_path}')
            break
